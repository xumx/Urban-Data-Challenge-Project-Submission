 ...
 * @type {string}
 */
// DDSimulator.MODEL_URL = 'http://sketchup.google.com/3dwarehouse/download?mid=c6bbc0a8a20d7b2c1475f5e36f75c973&rtyp=k2';
// DDSimulator.MODEL_URL = 'http://localhost:3333/tram.kmz';
...
function DDSimulator(ge, path, opt_opts) {
  ...
  if (!this.options.speed)
    this.options.speed = 3.0;

  if (!this.options.zoom)
    this.options.zoom = 300;
...
DDSimulator.prototype.finishInitUI_ = function(kml, opt_cb) {
  if (!kml || !kml.getFeatures().getChildNodes().getLength()) {
    console.log('Error loading Model KML. Expected Document > Placemark > Model.');
  }
  
  if ('getGeometry' in kml.getFeatures().getFirstChild() && kml.getFeatures().getFirstChild().getGeometry().getType() == 'KmlModel') {
    this.modelPlacemark = kml.getFeatures().getFirstChild();
  } else if ('getGeometry' in kml.getFeatures().getLastChild() && kml.getFeatures().getLastChild().getGeometry().getType() == 'KmlModel') {
    this.modelPlacemark = kml.getFeatures().getLastChild();
  }
...
DDSimulator.prototype.moveToPointDriving_ = function(loc, heading) {
  ...
  var range = curRange + (this.options.zoom - curRange) * 0.1 // range (inverse of zoom)
  var newRange = Math.max(this.options.zoom, range);
