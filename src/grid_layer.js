...
TimePlayer.prototype.render_time = function(tile, coord, zoom) {
	...
	// console.log(cells);
	var pixel_size = 2; //this.resolution//*this.options.cellsize;
	...
	if (self.sprite_2 == undefined) {
		...
		$(colors).each(function() {
			var canvas = document.createElement('canvas');
			var ctx = canvas.getContext('2d');
			ctx.width = canvas.width = 2;
			ctx.height = canvas.height = 2;
			ctx.globalAlpha = 0.75;
			ctx.fillStyle = this.toString();
			ctx.beginPath();
			ctx.arc(2, 2, 2, 0, tau, true, true);
			ctx.closePath();
			ctx.fill();
	...
	for (i = 0; i < len; ++i) {
		...
		if (this.options.trails == true) {
			...
			if (cell) {
				ctx.drawImage(self.sprite_2[cu], xc[i], yc[i])
			}

			cell = cells.values[this.MAX_UNITS * i + month - 2];
			if (cell) {
				ctx.drawImage(self.sprite_3[cu], xc[i], yc[i])
			}

			cell = cells.values[this.MAX_UNITS * i + month - 3];
			if (cell) {
				ctx.drawImage(self.sprite_4[cu], xc[i], yc[i])
			}
		}
	}
};
